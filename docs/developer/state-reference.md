...
Creator Analytics state is not stored in Redux. The current Analytics workspace uses local component state for the selected period and performance mode, with deterministic inspection fixtures when mock mode is enabled.
