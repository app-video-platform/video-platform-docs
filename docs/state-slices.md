...
|---|---|---|---|---|
| `auth` | `src/core/store/auth-store/*` | `setUserProfile`, `logout`, `resetOnboarding` | `selectAuthUser`, `selectAuthLoading`, `selectAuthError`, `selectIsUserLoggedIn` | Thunks: `signupUser`, `verifyEmail`, `signinUser`, `googleSignInUser`, `getUserProfile`, `logoutUser`, `updateUserDetails`. Sets `isUserLoggedIn`; triggers profile fetch post-login. Roles are backend-provided uppercase values and must not be changed locally. |
